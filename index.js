...
const TARGET_GROUP = TEST_GROUP_ID;
...
async function sendMessage(link, password = "") {
  const sendedMessage = await bot.sendMessage(TARGET_GROUP, link, { parse_mode: 'MarkDown' });
...
function check() {
  ...
  for (const [day] of Object.entries(config)) {
    for (const [time, link] of Object.entries(config[day])) {
      ...
      const date = new Date()
      if (date.getHours() == hour && date.getMinutes() == minute && date.getDay() == day) {
        sendMessage(link)
      }
    }
  }
